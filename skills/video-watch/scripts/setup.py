...
Modes:
  ...
  setup.py --json       Machine-readable status for the host to parse.
  setup.py              Reports missing deps or scaffolds .env on explicit invocation.
...
- Never install dependencies. Report exact commands for user-approved installation.
...
def _install_macos(missing: list[str]) -> tuple[bool, str]:
    ...
    return False, "dependencies missing. Install manually: brew install " + " ".join(pkgs)
...
def _status() -> dict:
    ...
    Whisper is an opt-in upload path. A keyless environment can proceed with
    public captions and frames as long as the local binaries are available.
    """
...
def cmd_install() -> int:
    ...
    if missing:
        ...
        if system == "Darwin":
            ...
            print(f"[setup] unsupported platform ({system}). Install manually:", file=sys.stderr)
