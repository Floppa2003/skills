#!/usr/bin/env python3
"""Video-watch entry point: download video, extract frames, parse transcript.

Prints a markdown report listing frame paths and a timestamped transcript.
"""
...
def main() -> int:
    ...
    ap.add_argument(
        "--allow-whisper",
        action="store_true",
        help="Allow an audio upload to the selected Whisper provider when captions are unavailable.",
    )
    ap.add_argument(
        "--no-whisper",
        action="store_true",
        help="Deprecated compatibility flag. Whisper is disabled unless --allow-whisper is set.",
    )
    ap.add_argument(
        "--whisper",
        choices=["groq", "openai"],
    ...
    args = ap.parse_args()
    if args.allow_whisper and args.no_whisper:
        ap.error("--allow-whisper and --no-whisper cannot be used together")
    if args.whisper and not args.allow_whisper:
        ap.error("--whisper requires --allow-whisper")
    ...
    if not transcript_segments and args.allow_whisper and video_path and meta.get("has_audio"):
        ...
        if backend and api_key:
            ...
            print(
                f"[watch] {hint} — add the key to ~/.config/watch/.env before retrying",
                file=sys.stderr,
            )
    elif not transcript_segments and video_path and meta.get("has_audio"):
        print(
            "[watch] captions unavailable; Whisper is disabled unless --allow-whisper is explicitly set",
            file=sys.stderr,
        )
    ...
    if frames:
        ...
        print()
        print(
            "**Inspect each frame path with the host image viewer.** "
    ...
    if transcript_text:
        ...
        print(
            ...
            "Captions were missing and Whisper was not explicitly authorized. "
            "Use `--allow-whisper` only after approving the audio upload._"
        )
